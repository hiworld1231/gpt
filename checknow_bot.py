...
def help_text():
    return ("<b>🤖 Hunter — управление</b>\n\n"
            "/menu — панель управления\n"
            "/checknow — проверить последние посты\n"
            "/checknow 100 — проверить 100\n"
            "/search opencode — найти посты по ключевому слову и проанализировать их\n"
...
async def search_messages(keyword, limit=30):
    keyword = keyword.strip()
    if not keyword:
        raise RuntimeError("укажи ключевое слово: /search opencode")
    if len(keyword) > 100:
        raise RuntimeError("ключевое слово слишком длинное")
    if not TG_API_ID or not TG_API_HASH or not TG_CHECKNOW_STRING_SESSION:
...
def analyze_without_db(title, text, url, published_at=None, mode=None):
    ...
    settings = load_settings(); mode = mode or settings["mode"]
    prompt = f"""{read_prompt()}\n\nРЕЖИМ ОХОТЫ: {mode}\n{MODES.get(mode, MODES['aggressive'])}\nТЕКУЩЕЕ ВРЕМЯ UTC: {datetime.now(timezone.utc).isoformat()}\nДАТА ПУБЛИКАЦИИ: {published_at or 'неизвестно'}\n\nВерни СТРОГО JSON без markdown. Поля: score (0-100), is_new (boolean), is_working (boolean), category (UPPER_SNAKE_CASE), summary (до 300 символов, русский), why (до 300), how (до 700; только из материала), risk (до 300).\n\nИсточник: {url}\nЗаголовок: {title}\n\nПОЛНЫЙ ДОСТУПНЫЙ МАТЕРИАЛ:\n{text[:50000]}"""
    last_error = None
    for model in MODELS:
        try:
            ...
            if last_error == "RATE_LIMIT":
                print(f"model {model} rate limited, trying next", flush=True); continue
            raise
    raise RuntimeError(f"all models unavailable: {last_error}")


def format_result(result, url, original_url=""):
    score = int(result.get("score", 0)); tier = "S-TIER" if score >= 90 else "A-TIER"
    ...
    if original_url and original_url != url: msg += f"\n🔎 <b>Оригинал:</b> {html.escape(original_url)}"
    return msg


def result_keyboard(message_id):
    return kb([[{"text": "🔍 Deep Check", "callback_data": f"deep:{message_id}"}, {"text": "✅ Verify", "callback_data": f"verify:{message_id}"}],
...
async def check_now(limit=None):
    settings = load_settings(); limit = limit or settings["limit"]; messages = await get_recent_messages(limit)
    send_message(f"🔎 Проверяю последние <b>{len(messages)}</b> текстовых постов @{html.escape(TG_SOURCE_CHANNEL)}...\n🎯 Score ≥ {settings['score']} · 🧠 режим: {html.escape(settings['mode'])}")
    sent = processed = 0
    for message in reversed(messages):
        text = (message.raw_text or "").strip()
        if not text: continue
        url = telegram_url(message); published_at = message.date.astimezone(timezone.utc).isoformat() if message.date else "неизвестно"
        try:
            original_url = extract_linuxdo_url(text)
            result = await asyncio.to_thread(analyze_without_db, telegram_title(text), f"[Дата публикации Telegram: {published_at}]\n{text}", url, published_at, settings["mode"])
            processed += 1
            if int(result.get("score", 0)) >= settings["score"]:
                send_message(format_result(result, url, original_url), result_keyboard(message.id)); sent += 1
        except Exception as e: print(f"check item {message.id}: {e}", flush=True)
        await asyncio.sleep(1.0)
    send_message(f"✅ <b>/checknow завершён</b>\n📊 Проанализировано: {processed}/{len(messages)}\n🔥 Найдено подходящих: {sent}")


async def search_now(keyword, limit=30):
    settings = load_settings()
    messages = await search_messages(keyword, limit)
    send_message(f"🔍 <b>Search: {html.escape(keyword)}</b>\nНайдено совпадений: <b>{len(messages)}</b>\n🤖 Анализирую найденные посты…")
    if not messages:
        send_message(f"❌ По <b>{html.escape(keyword)}</b> ничего не найдено.")
        return
    sent = processed = 0
    for message in reversed(messages):
        text = (message.raw_text or "").strip()
        url = telegram_url(message); published_at = message.date.astimezone(timezone.utc).isoformat() if message.date else "неизвестно"
        try:
            result = await asyncio.to_thread(analyze_without_db, telegram_title(text), f"[Поиск по ключевому слову: {keyword}]\n[Дата публикации Telegram: {published_at}]\n{text}", url, published_at, settings["mode"])
            processed += 1
            if int(result.get("score", 0)) >= settings["score"]:
                send_message(format_result(result, url, extract_linuxdo_url(text)), result_keyboard(message.id)); sent += 1
        except Exception as e: print(f"search item {message.id}: {e}", flush=True)
        await asyncio.sleep(0.5)
    send_message(f"✅ <b>Поиск завершён</b>\n🔎 Запрос: <code>{html.escape(keyword)}</code>\n📊 Проанализировано: {processed}/{len(messages)}\n🔥 Подходящих по Score ≥ {settings['score']}: {sent}")


def parse_int_arg(raw, default):
    try: return int(raw.split()[1]) if len(raw.split()) > 1 else default
    except Exception: return default


def show_menu_message(prefix=""):
    s = load_settings(); text = ("🤖 <b>Linux.do Hunter</b>\n\nВыбирай действие кнопками ниже.\n\n"
        f"🎯 Score: <b>{s['score']}</b>\n🔎 Limit: <b>{s['limit']}</b>\n🧠 Mode: <b>{html.escape(s['mode'])}</b>\n📡 Source: @{html.escape(TG_SOURCE_CHANNEL)}")
    if prefix: text = prefix + "\n\n" + text
    send_message(text, main_menu())


def handle_callback(update):
    cq = update["callback_query"]; data = cq.get("data", ""); message = cq.get("message") or {}
    if str((message.get("chat") or {}).get("id")) != CHAT_ID: return
    print(f"callback received: {data}", flush=True)
    try: answer_callback(cq["id"])
    except Exception as e: print(f"callback answer error: {e}", flush=True)
    message_id = message.get("message_id")
    try:
        if data == "menu:main": edit_message(message_id, "🤖 <b>Linux.do Hunter</b>\n\nВыбирай действие кнопками ниже.", main_menu())
        elif data == "menu:search": edit_message(message_id, "🔍 <b>Поиск по Telegram</b>\n\nНапиши команду:\n<code>/search opencode</code>\n\nИскать можно по любому слову или фразе. Поиск идёт по истории канала, затем найденные посты прогоняются через текущий LLM router.", kb([[{"text":"⬅️ Главное меню","callback_data":"menu:main"}]]))
        elif data == "menu:status": edit_message(message_id, status_text(), kb([[{"text":"⬅️ Главное меню","callback_data":"menu:main"}]]))
        elif data == "menu:help": edit_message(message_id, help_text(), kb([[{"text":"⬅️ Главное меню","callback_data":"menu:main"}]]))
        elif data == "menu:prompt":
            p = read_prompt(); preview = p[:2500] + ("\n…" if len(p) > 2500 else ""); edit_message(message_id, "📝 <b>Текущий prompt.txt</b>\n\n<pre>" + html.escape(preview) + "</pre>", kb([[{"text":"⬅️ Главное меню","callback_data":"menu:main"}]]))
        elif data == "menu:settings": edit_message(message_id, "⚙️ <b>Настройки</b>\n\nВыбирай параметр:", settings_menu())
        elif data == "menu:score": edit_message(message_id, "🎯 <b>Минимальный Score</b>\n\nНиже — быстрый выбор.", score_menu())
        elif data == "menu:limit": edit_message(message_id, "🔎 <b>Сколько постов проверять</b>\n\nМожно выбрать 20 / 50 / 75 / 100.", limit_menu())
        elif data == "menu:mode": edit_message(message_id, "🧠 <b>Режим охоты</b>\n\nВыбери режим.", mode_menu())
        elif data.startswith("score:"):
            value = max(0, min(int(data.split(":",1)[1]),100)); s=load_settings(); s["score"]=value; save_settings(s); edit_message(message_id,f"✅ Score установлен: <b>{value}</b>",settings_menu())
        elif data.startswith("limit:"):
            value=max(1,min(int(data.split(":",1)[1]),100)); s=load_settings(); s["limit"]=value; save_settings(s); edit_message(message_id,f"✅ Лимит установлен: <b>{value}</b>",settings_menu())
        elif data.startswith("mode:"):
            mode=data.split(":",1)[1]
            if mode in MODES:
                s=load_settings(); s["mode"]=mode; save_settings(s); edit_message(message_id,f"✅ Режим: <b>{html.escape(mode)}</b>\n\n{html.escape(MODES[mode])}",mode_menu())
        elif data == "alerts:toggle":
            s=load_settings(); s["alerts"]=not bool(s.get("alerts",True)); save_settings(s); edit_message(message_id,"⚙️ <b>Настройки</b>",settings_menu())
        elif data.startswith("check:"):
            limit=max(1,min(int(data.split(":",1)[1]),100)); answer_callback(cq["id"],f"Запускаю проверку {limit} постов"); asyncio.run(check_now(limit))
        else: print(f"unknown callback: {data}", flush=True)
    except Exception as e:
        print(f"callback {data}: {e}", flush=True)
        try: answer_callback(cq["id"], "Ошибка: смотри журнал бота", alert=True)
        except Exception: pass


def main():
    offset=None
    try: api_call("deleteWebhook", {"drop_pending_updates": False})
    except Exception as e: print(f"deleteWebhook: {e}", flush=True)
    print("Telegram command bot active: /menu /checknow /search /status /settings", flush=True)
    while True:
        try:
            ...
            for update in updates:
                ...
                if update.get("callback_query"):
                    try: handle_callback(update)
                    except Exception as e: print(f"callback handler: {e}", flush=True)
                    continue
                message=update.get("message") or {}; chat=message.get("chat") or {}
                if str(chat.get("id")) != CHAT_ID: continue
                raw=(message.get("text") or "").strip(); command=raw.split()[0].lower() if raw else ""
                if command == "/menu": show_menu_message()
                elif command in ("/checknow","/check"):
                    try: asyncio.run(check_now(max(1,min(parse_int_arg(raw,load_settings()["limit"]),100))))
                    except Exception as e: print(f"checknow: {e}",flush=True); send_message(f"❌ /checknow: <code>{html.escape(str(e)[:700])}</code>")
                elif command in ("/search","/find"):
                    try:
                        parts=raw.split(maxsplit=1)
                        keyword=parts[1].strip() if len(parts)>1 else ""
                        asyncio.run(search_now(keyword, min(max(int(os.getenv("SEARCH_LIMIT","30")),1),100)))
                    except Exception as e: print(f"search: {e}",flush=True); send_message(f"❌ /search: <code>{html.escape(str(e)[:700])}</code>")
                elif command == "/score":
                    value=parse_int_arg(raw,load_settings()["score"])
                    if len(raw.split())>1: s=load_settings(); s["score"]=max(0,min(value,100)); save_settings(s); send_message(f"✅ Score: <b>{s['score']}</b>",main_menu())
                    else: send_message("🎯 <b>Score</b>",score_menu())
                elif command == "/limit":
                    value=parse_int_arg(raw,load_settings()["limit"])
                    if len(raw.split())>1: s=load_settings(); s["limit"]=max(1,min(value,100)); save_settings(s); send_message(f"✅ Limit: <b>{s['limit']}</b>",main_menu())
                    else: send_message("🔎 <b>Лимит</b>",limit_menu())
                elif command == "/mode":
                    parts=raw.split()
                    if len(parts)>1 and parts[1] in MODES: s=load_settings(); s["mode"]=parts[1]; save_settings(s); send_message(f"✅ Mode: <b>{parts[1]}</b>",main_menu())
                    else: send_message("🧠 <b>Режим</b>",mode_menu())
                elif command == "/status": send_message(status_text(),main_menu())
                elif command == "/settings": send_message("⚙️ <b>Настройки</b>",settings_menu())
                elif command == "/prompt":
                    p=read_prompt(); preview=p[:2500]+("\n…" if len(p)>2500 else ""); send_message("📝 <b>prompt.txt</b>\n\n<pre>"+html.escape(preview)+"</pre>",main_menu())
                elif command in ("/help","/start"): show_menu_message(help_text())
        except Exception as e: print(f"command bot: {e}",flush=True); time.sleep(5)
